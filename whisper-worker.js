...
self.onmessage = async function(e) {
    if (e.data.type === 'load') {
        try {
            self.postMessage({type:'progress', msg:'Loading...', pct:0});
            ...
            if (isIOS) {
                ...
                self.postMessage({type:'progress', msg:'Whisper Tiny (iOS)...', pct:5});
            } else if (useWebGPU) {
            ...
            try {
                transcriber = await pipeline('automatic-speech-recognition', modelId, {
                    ...
                    progress_callback: function(p) {
                        if (p.status === 'progress' && p.total) {
                            ...
                            self.postMessage({type:'progress', msg:'Model: ' + pct + '%', pct: pct});
                        }
                    }
                });
            } catch(modelErr) {
                if (useWebGPU) {
                    ...
                    self.postMessage({type:'progress', msg:'WebGPU unavailable, CPU...', pct:5});
                    modelId = 'onnx-community/whisper-base';
                    totalFiles = {};
                    transcriber = await pipeline('automatic-speech-recognition', modelId, {
                        dtype: { encoder_model: 'fp32', decoder_model_merged: 'q4' },
            ...
            self.postMessage({type:'error', msg:'Error: ' + err.message});
        }
    }

    if (e.data.type === 'transcribe') {
        if (!transcriber) { self.postMessage({type:'error', msg:'Model not loaded', id:e.data.id}); return; }
        try {
