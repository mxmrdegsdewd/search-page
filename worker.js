...
var SENT_MARKERS=new Set('ну вот короче слушай слушайте смотри смотрите кстати потом далее затем впрочем итак значит получается оказывается давай давайте ладно хорошо окей так нет да стоп подожди'.split(' '));
...
function isQuestion(sent,allWords,allTypes,from,to){if(!sent)return false;var w=sent.replace(/[.,?!;:]/g,'').trim().split(/\s+/);if(!w.length)return false;if(Q_WORDS.has(w[0]))return true;var joined=w.join(' ');for(var i=0;i<Q_PARTICLES.length;i++){if(joined.indexOf(Q_PARTICLES[i])!==-1)return true}for(var j=from;j<to&&j<allWords.length;j++){if(allTypes[j]==='VERB2')return true}return false}
...
function punctuate(raw,isFinal){
    ...
    for(var i=0;i<words.length;i++){
        var w=words[i],type=types[i],prev=i>0?words[i-1]:'',prevType=i>0?types[i-1]:'',next=i<words.length-1?words[i+1]:'',distFromStart=i-sentStart;
        ...
        if(i>0&&SUBORDINATE.has(w)&&!isPartOfCompound(words,i)){if(result.length>0&&!ep(result[result.length-1])&&prev!=='и'&&prev!=='или'&&prev!=='не'&&prev!=='то'&&!PREPOSITIONS.has(prev))result[result.length-1]+=','}
        if(i>0&&COORD_CONJ.has(w)&&result.length>0&&!ep(result[result.length-1]))result[result.length-1]+=',';
        if(i>0&&distFromStart>=3&&SENT_MARKERS.has(w)&&next&&!NO_BREAK.has(next)&&!SUBORDINATE.has(next)){if(result.length>0&&!ep(result[result.length-1])){var ps=result.slice(sentStart).join(' ');result[result.length-1]+=isQuestion(ps,words,types,sentStart,i)?'?':'.'}sentStart=result.length;lastVerbIdx=-1}
        if(i>0&&(type==='VERB'||type==='VERB2')&&distFromStart>=4&&lastVerbIdx>=sentStart&&lastVerbIdx<i-2&&!SUBORDINATE.has(prev)&&!COORD_CONJ.has(prev)&&prev!=='и'&&prev!=='или'){if(result.length>0&&!ep(result[result.length-1])){var ps2=result.slice(sentStart).join(' ');result[result.length-1]+=isQuestion(ps2,words,types,sentStart,i)?'?':'.'}sentStart=result.length}
        result.push(w);
    ...
    if(isFinal&&result.length>0&&!ep(result[result.length-1])){var fs=result.slice(sentStart).join(' ');if(isQuestion(fs,words,types,sentStart,words.length))result[result.length-1]+='?'}
    var out=result.join(' ');out=out.replace(/,\s*,/g,',').replace(/\.\s*\./g,'.').replace(/,\./g,'.').replace(/,\?/g,'?').replace(/\?\./g,'?').replace(/ +/g,' ').trim();
